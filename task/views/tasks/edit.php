...
<div class="dashboard-container">
    ...
    <main class="main-content">
        ...
        <div class="task-form-container">
            <form method="POST" action="edit.php?id=<?php echo $task['id']; ?>" class="task-form">
                ...
                <div class="form-row">
                    <div class="form-group col-md-6">
                        ...
                        <input type="date" id="date_debut" name="date_debut" class="form-control" required
                               value="<?php echo date('Y-m-d', strtotime($task['date_debut'])); ?>">
                    </div>
                    <div class="form-group col-md-6">
                        <label for="heure_debut">Heure de début</label>
                        <input type="time" id="heure_debut" name="heure_debut" class="form-control" required
                               value="<?php echo date('H:i', strtotime($task['heure_debut'])); ?>">
                    </div>
                </div>
                ...
                <div class="form-row">
                    <div class="form-group col-md-6">
                        ...
                        <input type="date" id="date_echeance" name="date_echeance" class="form-control" required
                               value="<?php echo date('Y-m-d', strtotime($task['date_echeance'])); ?>">
                    </div>
                    <div class="form-group col-md-6">
                        ...
                        <input type="time" id="heure_echeance" name="heure_echeance" class="form-control" required
                               value="<?php echo date('H:i', strtotime($task['heure_echeance'])); ?>">
                    </div>
                </div>
                    ...
                    <select id="id_categorie" name="id_categorie" class="form-control" required>
                        <?php while ($categorie = $categories->fetch(PDO::FETCH_ASSOC)): ?>
                            <option value="<?php echo $categorie['id']; ?>" 
                                    <?php echo ($task['id_categorie'] == $categorie['id']) ? 'selected' : ''; ?>>
